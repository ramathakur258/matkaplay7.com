...
//         	$sql="SELECT (SELECT sum(`amount`) FROM `users_wallet` WHERE `from_user_id`=`users`.`user_id` AND `txn_type`='credit') as credit1 , (SELECT sum(`amount`) as credit1 FROM `users_wallet` WHERE `to_user_id`=`users`.`user_id` AND `txn_type`='debit') as credit2 ,(SELECT sum(`amount`) FROM `users_wallet` WHERE `from_user_id`=`users`.`user_id` AND `txn_type`='debit') as debits FROM `users` WHERE users.user_id=".$user_id;
...
if(!function_exists('GetRewardBalance')){
    function GetRewardBalance($user_id){
        $ci =& get_instance();
        $ci->load->database();
        
        	$sql="SELECT (SELECT sum(`amount`) FROM `rewards` WHERE `user_id`=`users`.`user_id` AND `txn_type`='credit') as credit ,(SELECT sum(`amount`) FROM `rewards` WHERE `user_id`=`users`.`user_id` AND `txn_type`='debit') as debit FROM `users` WHERE users.user_id=".$user_id;
        $result=$ci->db->query($sql);
        $balance=0.00;
        if($result->num_rows() > 0)
        {
